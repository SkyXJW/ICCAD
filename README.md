...
- 以 v6 的提交/评测包装为基础：保留 `configs/cada1078_alpha_submit.yml`、stdin 单条 prompt 异常护栏、日志目录 fallback。
...
├── configs/
│   ├── cada1078_alpha_submit.yml   # 评测/提交推荐配置（无硬编码路径）
│   └── cada1078_alpha_develop.yml  # 本地开发配置（不要提交到 Docker context）
...
- `iverilog`
- `yosys`
- `abc` / `berkeley-abc` / `yosys-abc`（深度优化与 CEC 等价验证依赖）

正式提交的 `cada1078_alpha` 会在 PyInstaller executable 内嵌上述 EDA tools、Yosys data、Icarus `ivl` data 与必要的非 glibc runtime libraries；TSRI 评测机不需要预装这些 EDA binaries。
...
```bash
export YOSYS_BIN=yosys          # Yosys 路径，默认从 PATH 查找 yosys
export ABC_BIN=abc             # 普通 ABC 二进制路径；默认依次查找 abc/berkeley-abc/yosys-abc
export ABC_CEC_BIN=yosys-abc   # CEC 优先使用的 ABC，可不设；程序会自动 fallback
export ABC_TIMEOUT=600         # 单次 abc 调用超时（秒），默认 120；大用例建议调大
export IVERILOG_BIN=iverilog   # Verilog 预处理器路径；PyVerilog 仍要求其可执行文件在运行环境可用
...
```bash
PYTHONPATH=src python3 src/contest_agent.py -config configs/cada1078_alpha_submit.yml
```
...
`cada1078_alpha` 由 `packaging/build_pyinstaller.sh` 生成；构建脚本会先 staging `yosys`、`berkeley-abc`/`yosys-abc`、`iverilog`/`vvp` 及其资源，再打入单文件 executable。Docker 仅作为可复现构建环境，最终提交/评测不应要求 `docker run`、`pip install`、联网安装 packages 或系统预装 EDA tools。
